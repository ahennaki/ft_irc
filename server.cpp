...
Server::Server(std::string port, std::string password) : port(port), password(password) {
	if (port.find_first_not_of("0123456789") != std::string::npos)
		throw (std::runtime_error("invalide <port>: \"should be integer\"!"));
	int prt = toInt(port);
	if (prt < 0 || prt > 65535)
		throw (std::runtime_error("invalide <port>: \"should be between 0 and 65535\"!"));
...
void Server::acceptClient()
{
	...
	pfd.push_back(NewPoll);

	std::cout << "Client Connected: " << incofd << std::endl;
}
...
void Server::ClearClients(int fd) {
	for (size_t i = 0; i < pfd.size(); i++) {
		if (pfd[i].fd == fd) {pfd.erase(pfd.begin() + i); break;}
	}
